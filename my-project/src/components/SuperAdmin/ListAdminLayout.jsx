...
function ListAdminLayout() {
  ...
  const fetchAdmins = async () => {
    try {
      ...
      const response = await fetch(
        "https://keluh-dev.mdrizki.my.id/api/v1/admins",
        {
          method: "GET",
          headers: {
  ...
  const deleteAdmin = async (adminId) => {
    try {
      ...
      const response = await fetch(
        `https://keluh-dev.mdrizki.my.id/api/v1/admins/${adminId}`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleNewAdminInputChange = (e) => {
    // Fungsi untuk menangani input pada form tambah akun baru
    const { name, value } = e.target;
  ...
  const handleEditSubmit = async (e) => {
    ...
    try {
      const token = sessionStorage.getItem("token");
      const response = await fetch(
        `https://keluh-dev.mdrizki.my.id/api/v1/admins/${currentAdmin.id}`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
            ...
            <table className="table-auto w-full font-poppins">
              ...
              <tbody className="py-4 w-full">
                {admins.map((admin, index) => (
                  <tr
                    key={admin.id}
                    className="text-black bg-white text-center border-b border-black first:pt-4 last:pb-4"
                  >
            ...
            <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50">
              ...
              >
                ...
                <div className="flex justify-end">
                  ...
                  <button
                    type="submit"
                    className="px-4 py-2 bg-main-color hover:bg-main-darker text-white rounded"
                  >
                    Save
                  </button>
                </div>
